...
#             self.repo_data.append(self.fetchPrData(pr))
    ...
    def classify(self):
        ...
        for pr_nr in self.repo_data:
            ...
            parent = self.repo_data[pr_nr]['first_commit_parent']
            sha = self.repo_data[pr_nr]['last_commit_sha']
                ...
                for file in files:
                    ...
                    if len(files[file]) != 0:
                        try:
                            ...
                            destPath, destUrl_ = classifier.getFileFromDest(self.repo_dir_files, self.variant2, destination_sha, self.repo_file, file, new_file_dir, fileName, file_ext, self.token_list[self.ct])
                            self.ct += 1

                            if status =='added':
                                """
                                    Get the file after the patch from variant1
                                """
                                if self.ct == self.len_tokens:
                                    self.ct = 0
                                fileAfterPatchDir, fileAfterPatchUrlAdd_= classifier.getFileAfterPatch(self.repo_dir_files, self.variant1, sha, self.repo_file, pr_nr, file, new_file_dir, fileName, file_ext, self.token_list[self.ct])
                                self.ct += 1
